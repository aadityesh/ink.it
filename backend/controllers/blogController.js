...
export const updateBlog = catchAsyncErrors(async (req, res, next) => {
    ...
    if (req.files) {
        ...
        if (
            ...
            return next(new ErrorHandler("Invalid format, allowed formats - png/jpg/jpeg/webp", 404))
        }

        if (req.files && mainImage) {
            const blogMainImageID = blog.mainImage.public_id
            const newBlogMainImage = await cloudinary.uploader.destroy(blogMainImageID)
...
// export const delete = catchAsyncErrors(async (req, res, next) => { })
